...
public interface VimeoService {
    ...
    void authenticateWithCodeGrant(@Field("redirect_uri") String redirectURI,
    ...
    @POST("/users")
    void join(@Body HashMap<String, String> parameters,
              Callback<Account> callback);
    ...
    @FormUrlEncoded
    @POST("/oauth/authorize/password")
    Account logIn(@Field("username") String email,
                  @Field("password") String password,
                  @Field("grant_type") String grantType,
                  @Field("scope") String scope);
    ...
    @GET("/videos")
    void searchVideos(@Query("query") String query,
